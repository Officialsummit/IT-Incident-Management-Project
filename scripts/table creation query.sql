...
CREATE TABLE dim_date (
    idDate INT PRIMARY KEY IDENTITY(1,1) NOT NULL,
...
CREATE TABLE dim_Reported (
    idIncident INT PRIMARY KEY IDENTITY(1,1) NOT NULL,
...
CREATE TABLE fact_Incident_Management (
    ...
    CONSTRAINT FK_dim_Reported FOREIGN KEY (idIncident) REFERENCES dim_Reported(idIncident),
...
--use incident_test
use servicenow_incidents_db
--Delete From staging_table
--Delete  from fact_Incident_Management
--Delete  from dim_category
--Delete  from dim_status
--Delete  from dim_date
--Delete  from dim_employee
--Delete  from dim_Reported
...
SELECT  *  FROM incident_test.[dbo].[dim_employee]
...
SELECT  *    FROM dim_reported 
	   
SELECT  *    FROM dim_date where date = null

SELECT  count(*)    FROM fact_incident_management where Updated_by = 'UpdatedBy908'

select a.idDate, a.date from dim_date a join fact_Incident_Management b 
on a.idDate = b.Created_at 
where b.idIncident = '24919'
